...
public class UpdateController implements Initializable {
    @FXML
    public ChoiceBox<String> selectMaterialChoiceBox;
    @FXML
    public TextField valueUpdateTextField;
    @FXML
    public TextField idUpdateTextField;
    @FXML
...

    {
        ...
        MATERIAL.put("city", "city");
        MATERIAL.put("phone", "phone");
        MATERIAL.put("name", "fullname");
        MATERIAL.put("department", "department");
    ...
    public void materialRegistration(ActionEvent event) throws Exception {  //to move to insert page
    ...
    public String getSelectMaterialChoiceBox() {  //return code of material name
        for (Map.Entry<String, String> entry : MATERIAL.entrySet()) {
            if (selectMaterialChoiceBox.getValue().equals(entry.getValue()))
                return (entry.getKey());
        }
        return null;
    }

    public boolean checkIsNumber(String string) {  //used to check if string is number or not
        try {
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        for (Map.Entry<String, String> entry : MATERIAL.entrySet())
            selectMaterialChoiceBox.getItems().addAll(entry.getValue());
    }

    public boolean studentUpdate() {
        if (idUpdateTextField.getText().isEmpty()) {    //check if text field is empty
            updateLabel.setText("id is requirement !");
        } else if (valueUpdateTextField.getText().isEmpty()) {  //check if text field is empty
            updateLabel.setText("value is requirement !");
        } else if (selectMaterialChoiceBox.getValue().isEmpty()) {  //check if choice box is empty
            updateLabel.setText("select item from ChoiceBox is requirement !");
        } else if (search(idUpdateTextField.getText())) {     //if(all) not empty search with id in database

            if (checkIsNumber(valueUpdateTextField.getText())) {
                /*
                check if entered value is valid or not
                */

                if (getSelectMaterialChoiceBox().equals("city") || getSelectMaterialChoiceBox().equals("fullname") || getSelectMaterialChoiceBox().equals("department")) {
                    updateLabel.setText("value not match !");
                    return false;
                } else if (Integer.parseInt(valueUpdateTextField.getText()) < 0 || Integer.parseInt(valueUpdateTextField.getText()) > 100) {
                    updateLabel.setText("value mast be between 0&100");
                    return false;
                }
            }
            try {
                Connection conn = studentConnection();
                String sql_column = getSelectMaterialChoiceBox();
                String sql_query = "update students set " + sql_column + "=? where id=?";
                PreparedStatement stmt = conn.prepareStatement(sql_query);
                stmt.setString(1, valueUpdateTextField.getText());
                stmt.setString(2, idUpdateTextField.getText());
                stmt.executeUpdate();
                updateLabel.setText("Updating is done");
                conn.close();

            } catch (SQLException e) {
                e.printStackTrace();
                updateLabel.setText("problem with connection ");
            }
        } else {
            updateLabel.setText("id not found ");
        }
        return true;
    }
}
